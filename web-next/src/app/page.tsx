...
import { VisualizationSwitch } from "@/components/visualization/VisualizationSwitch";
...
function JarvisInterface() {
  ...
  const {
    ...
    setShowCalendar,
    setAudioLevel,
    setWakeWordTriggered,
    setWakeWordTimestamp,
  } = useJarvisStore();
  ...
  const isConfigured =
    provider === "cartesia"
      ? !!groqApiKey && (ttsProvider === "edge" || !!cartesiaApiKey)
      : provider === "groq"
        ? !!groqApiKey
        : provider === "elevenlabs"
          ? !!elevenLabsAgentId
  ...
  // Demo mode - cycles through states to showcase visualization effects
  useEffect(() => {
    if (!demoMode) return;
    ...
    // Trigger wake-word effect at start
    setWakeWordTriggered(true);
    setWakeWordTimestamp(Date.now());
    ...
    const stateInterval = setInterval(() => {
      ...
      // Trigger ripple on each state change
      if (stateIndex === 1) {
        // listening
        setWakeWordTriggered(true);
        setWakeWordTimestamp(Date.now());
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Demo mode toggle with 'D' key
      if (e.code === "KeyD" && !showSettings && e.target === document.body) {
        e.preventDefault();
      ...
      if (e.code === "Space" && !showSettings && e.target === document.body) {
        ...
        if (isConnected) {
          ...
          if (
            state === "speaking" &&
            "interrupt" in currentProvider &&
            typeof (currentProvider as { interrupt?: () => void }).interrupt ===
              "function"
          ) {
      ...
      <div className="absolute top-6 left-6 z-20 flex flex-col gap-2">
        ...
        {demoMode && (
          <div className="flex items-center gap-2 animate-pulse">
            <span
              className={`text-xs ${isDark ? "text-orange-400" : "text-orange-600"} flex items-center gap-1 font-medium`}
            >
              <svg
        ...
        {lastMemoryRetrieved && memoryEnabled && (
          <div className="flex items-center gap-2 animate-fade-in">
            <span
              className={`text-xs ${isDark ? "text-blue-400" : "text-blue-600"} flex items-center gap-1`}
            >
              <svg
                className="w-3.5 h-3.5"
                fill="currentColor"
              ...
              {lastMemoryRetrieved.count}{" "}
              {lastMemoryRetrieved.count === 1 ? "memory" : "memories"} found
            </span>
          </div>
        )}
        ...
        {lastMemorySaved && memoryEnabled && (
          <div className="flex items-center gap-2 animate-fade-in">
            <span
              className={`text-xs ${isDark ? "text-emerald-400" : "text-emerald-600"} flex items-center gap-1`}
            >
              <svg
                className="w-3.5 h-3.5"
                fill="currentColor"
      ...
      <div className="absolute top-[8%] left-1/2 -translate-x-1/2 z-10 w-full max-w-2xl px-6">
        ...
        {state === "thinking" && (
          <div className="flex justify-center items-center gap-1 mt-3">
            <span
              className={`w-2 h-2 rounded-full ${isDark ? "bg-cyan-400" : "bg-cyan-500"} animate-bounce`}
