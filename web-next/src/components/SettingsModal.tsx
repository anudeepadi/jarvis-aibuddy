...
import { useJarvisStore, OpenAIVoice, CartesiaVoice, EdgeVoice, GeminiVoice, TTSProvider, Theme, Language } from '@/store/jarvis-store'
...
const GEMINI_VOICES: { id: GeminiVoice; name: string; description: string }[] = [
  { id: 'Puck', name: 'Puck', description: 'Playful & witty' },
  { id: 'Charon', name: 'Charon', description: 'Deep & mysterious' },
  { id: 'Kore', name: 'Kore', description: 'Warm & gentle' },
  { id: 'Fenrir', name: 'Fenrir', description: 'Bold & powerful' },
  { id: 'Aoede', name: 'Aoede', description: 'Musical & flowing' },
  { id: 'Orbit', name: 'Orbit', description: 'Clear & modern' },
  { id: 'Clio', name: 'Clio', description: 'Storytelling' },
  { id: 'Zephyr', name: 'Zephyr', description: 'Light & breezy' },
  { id: 'Nova', name: 'Nova', description: 'Bright & energetic' },
  { id: 'Aria', name: 'Aria', description: 'Elegant & refined' },
]
...
export function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
  ...
  const handleSave = () => {
    ...
    setApiKey('geminiApiKey', localKeys.geminiApiKey)
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button
              ...
              className={`p-3 rounded-xl border transition-all col-span-2 ${
                provider === 'gemini-live'
                  ? 'border-cyan-500 bg-cyan-500/10 text-white'
                  : 'border-gray-700 hover:border-gray-600 text-gray-300'
              }`}
            >
              <div className="font-medium text-sm">Gemini Live</div>
              <div className="text-xs text-blue-400">Native Audio - 30 HD Voices</div>
            </button>
          </div>
        </div>
          ...
          <div className="space-y-4 mb-6">
            <div>
              ...
              <input
                ...
                onChange={(e) => setLocalKeys({ ...localKeys, geminiApiKey: e.target.value })}
                placeholder="AIzaSy..."
                className="w-full px-3 py-2.5 rounded-lg bg-[#0d0d0d] border border-gray-700 text-white placeholder-gray-600 focus:border-cyan-500 focus:outline-none font-mono text-sm"
              />
              <p className="mt-1.5 text-xs text-gray-500">
                Get from aistudio.google.com - Native audio bidirectional streaming
              </p>
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-400 uppercase tracking-wide mb-2">
              ...
              <div className="grid grid-cols-5 gap-2">
                {GEMINI_VOICES.map((voice) => (
                  ...
                  >
                    <div className={`text-sm font-medium ${geminiVoice === voice.id ? 'text-white' : 'text-gray-300'}`}>
                      {voice.name}
                    </div>
            ...
            <div className="p-3 rounded-lg bg-blue-500/10 border border-blue-500/20">
              <div className="text-sm text-blue-400 font-medium">Native Audio Features</div>
              <ul className="mt-1 text-xs text-blue-300/70 space-y-0.5">
                <li>• Bidirectional audio streaming (no STT/TTS)</li>
                <li>• Proactive audio & affective dialog</li>
                <li>• Natural barge-in (interrupt anytime)</li>
                <li>• 30 HD voices in 24 languages</li>
              </ul>
            </div>
          </div>
        )}
